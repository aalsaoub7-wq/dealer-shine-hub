  ...
  const handleInteriorEdit = async (color: string) => {
    ...
    const processNextInterior = async (): Promise<void> => {
      ...
      try {
          ...
          const transparentPublicUrl = `${segmentData.url}?t=${Date.now()}`;
      ...
      <div className="container mx-auto px-3 md:px-4 py-4 md:py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 md:space-y-6 animate-fade-in-up">
          <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:justify-between">
            <TooltipProvider>
              <TabsList className="bg-card border border-border shadow-card w-full md:w-auto">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <span className="flex-1 md:flex-none">
                      <TabsTrigger
                        value="main"
                        className="data-[state=active]:bg-gradient-primary data-[state=active]:text-primary-foreground transition-all duration-300 text-xs md:text-sm w-full"
                      >
        ...
        onImageSelected={(imageUrl) => {
          ...
          photosToProcess.forEach(async (photo, index) => {
            ...
            try {
              ...
              const cacheBustedUrl = `${segmentData.url}?t=${Date.now()}`;
              await supabase.from("photos").update({ transparent_url: cacheBustedUrl, original_url: photo.url }).eq("id", photo.id);
              ...
              if (index === 0 && interiorFlowId === editFlowIdRef.current) {
                setPositionEditorPhoto({
                  id: photo.id,
                  transparentCarUrl: cacheBustedUrl,
                  editType: 'interior',
                  backgroundImageUrl: imageUrl,
                  moveBackground: true,
                  flowId: interiorFlowId,
