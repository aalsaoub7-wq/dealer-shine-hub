...
const Auth = () => {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-card to-background p-4">
      <Card className="w-full max-w-md bg-gradient-card border-border/50 shadow-glow">
        ...
        <CardContent>
          {isForgotPassword ? resetEmailSent ? <div className="text-center space-y-4">
            ...
            setResetEmailSent(false);
                ...
                <div className="space-y-2">
                  ...
                  <Input id="reset-email" type="email" placeholder="namn@företag.se" value={email} onChange={e => setEmail(e.target.value)} disabled={loading} className={errors.email ? "border-destructive" : ""} />
                  {errors.email && <p className="text-sm text-destructive">{errors.email}</p>}
                </div>
                <Button type="submit" className="w-full" disabled={loading}>
                  {loading ? "Skickar..." : "Skicka återställningslänk"}
                </Button>
                <Button type="button" variant="ghost" onClick={() => setIsForgotPassword(false)} className="w-full" disabled={loading}>
                  Tillbaka till inloggning
                </Button>
              </form> : <>
              {!isLogin && <Alert className="mb-4 border-primary/20 bg-primary/5">
              ...
              <Button type="button" variant="outline" className="w-full bg-background hover:bg-accent border-border" onClick={handleGoogleSignIn} disabled={loading}>
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="email" type="email" placeholder="namn@företag.se" value={email} onChange={e => setEmail(e.target.value)} disabled={loading} className={errors.email ? "border-destructive" : ""} />
                  {errors.email && <p className="text-sm text-destructive">{errors.email}</p>}
                </div>
                <div className="space-y-2">
                  ...
                  {isLogin && <button type="button" onClick={() => setIsForgotPassword(true)} className="text-sm text-primary hover:underline" disabled={loading}>
                      Glömt lösenord?
                    </button>}
                </div>
                {!isLogin && <div className="space-y-2">
                    ...
                    <Input id="inviteCode" type="text" placeholder="Ange kod från din arbetsgivare" value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} disabled={loading} className={errors.inviteCode ? "border-destructive" : ""} />
