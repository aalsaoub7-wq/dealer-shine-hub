...
const SortablePhotoCard = ({
  ...
  return <Card ref={setNodeRef} style={style} className={`bg-gradient-card border-border/50 overflow-hidden group shadow-card md:hover:shadow-intense md:hover:-translate-y-2 transition-all duration-500 animate-fade-in-up ${isSelected ? 'ring-2 ring-primary' : ''}`} {...attributes}>
      <div className="relative aspect-video bg-secondary">
        ...
        <div className={`absolute bottom-2 left-2 z-10 transition-opacity ${<div className={`absolute bottom-2 left-2 z-10 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-100 md:opacity-0 md:group-hover:opacity-100'}`} onClick={e => e.stopPropagation()} onPointerDown={e => {}`} onClick={e => e.stopPropagation()} onPointerDown={e => {
