...
serve(async (req) => {
  ...
  try {
    console.log(`[CREATE-STRIPE-CUSTOMER] Creating customer`);
    ...
    // Create Stripe customer (no subscription - that's set up manually per contract)
    ...
    return new Response(
      JSON.stringify({
        ...
        message: "Customer created successfully - subscription will be set up manually",
      }),
      {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
